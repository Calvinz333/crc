"""
Manuscript → PDF (manuscript-based, with corrected figures embedded)
Reads: reports/Manuscript_Draft.md
Embeds: figures/publication_ready/fig1–4 at the correct Results sections
Output: reports/Manuscript_Draft.pdf
"""
...
PDF_OUT = os.path.join(BASE, "reports", "Manuscript_Draft.pdf")
FIG_DIR = os.path.join(BASE, "figures", "publication_ready")

FIGURES = {
    "fig1": (os.path.join(FIG_DIR, "fig1_pca_combat.png"),
             "<b>Figure 1 — PCA: Before vs After ComBat Correction.</b> "
             "(A) Raw CLR data coloured by cohort: tight study-specific clusters dominate PC1–PC2. "
             "(B) After ComBat: study clusters dissolve; CRC/CTR disease separation is preserved."),
    "fig2": (os.path.join(FIG_DIR, "fig2_roc_curves.png"),
             "<b>Figure 2 — Diagnostic Performance.</b> "
             "(A) Overlaid ROC curves: Internal 5-fold CV (AUC = 0.900) vs LODO external mean (AUC = 0.785). "
             "(B) Per-cohort LODO AUC; US-CRC-2 ensemble AUC = 0.606 (RF = 0.603, XGBoost = 0.573, LightGBM = 0.588)."),
    "fig3": (os.path.join(FIG_DIR, "fig3_shap_interaction.png"),
             "<b>Figure 3 — SHAP Importance and Epistatic Synergy.</b> "
             "(A) Mean |SHAP| bar; red = CRC-enriched, blue = CTR-enriched. "
             "(B) P. micra × Anaerotruncus sp. interaction scatter (interaction coeff = +0.147, FDR &lt; 0.001)."),
    "fig4": (os.path.join(FIG_DIR, "fig4_geographic_attenuation.png"),
             "<b>Figure 4 — Geographic Attenuation in US-CRC-2.</b> "
             "(A) Fold-change attenuation of top-10 European SHAP taxa; lead Firmicutes sp. = 30× drop "
             "(p = 2.3 × 10\u207b\u00b9\u00b9). (B) Paired median CLR: European cohorts vs US-CRC-2."),
}

# Which Results sub-heading triggers each figure
FIG_TRIGGER = {
...
ST = {
    ...
    "code":    ps("CO", fontSize=8, leading=11, textColor=colors.HexColor("#1565C0"),
...
def parse_md(text):
    ...
    def flush_table():
        if tbl_buf:
            ...
            if t:
                story.append(Spacer(1, 6)); story.append(t); story.append(Spacer(1, 8))
            tbl_buf.clear()
    ...
    while i < len(lines):
        ...
        if re.match(r'^# [^#]', line):
            flush_bullets()
            story.append(Paragraph(mdi(line[2:].strip()), ST["title"])); i += 1; continue
        ...
        if re.match(r'^### [^#]', line):
            ...
            # Embed figure after matching sub-heading
        ...
        if re.match(r'^#### [^#]', line):
            flush_bullets()
            story.append(Paragraph(mdi(line[5:].strip()), ST["h3"])); i += 1; continue
        ...
        if line.strip().startswith("$$"):
            flush_bullets()
            math = [line.strip().strip("$")]
            i += 1
            while i < len(lines) and not lines[i].strip().startswith("$$"):
                math.append(lines[i].strip()); i += 1
            i += 1
            formula = " ".join(l for l in math if l).replace("$","").strip()
            story.append(Paragraph(formula, ST["code"])); continue
...
def build():
    ...
    doc = SimpleDocTemplate(
        ...
        title="CRC Metagenomics — Microbiome Manuscript",
    ...
    for k,(p,_) in FIGURES.items():
        print(f"    {k}: {'✓' if os.path.exists(p) else '✗ MISSING'}")
